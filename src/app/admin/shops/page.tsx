...
export default async function AdminShopsPage() {
  ...
  const viewModels = shops.map((shop) => { const shopId = String(shop.id); const shopProducts = productRows.filter((product) => String(product.shop_id) === shopId); const shopOrdersForShop = orderRows.filter((order) => String(order.shop_id) === shopId); const owner = profileMap.get(String(shop.owner_id)); const ownerName = authNameMap.get(String(shop.owner_id)) || owner?.full_name || owner?.display_name || shop.owner_name || shop.owner_email || "Owner profile unavailable"; return { id: shopId, name: String(shop.name ?? "Unnamed shop"), slug: String(shop.slug ?? ""), status: String(shop.status ?? "pending").toLowerCase(), owner: String(ownerName), email: String(shop.created_at ?? ""), logo: String(shop.logo_url ?? shop.logo ?? ""), products: shopProducts.length, orders: shopOrdersForShop.length, sales: shopOrdersForShop.filter((order) => ["delivered", "completed"].includes(String(order.order_status ?? order.status ?? "").toLowerCase())).reduce((sum, order) => sum + orderAmount(order, itemTotals.get(String(order.id)) ?? 0), 0) }; });
