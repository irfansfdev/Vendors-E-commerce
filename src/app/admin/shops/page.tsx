...
export default async function AdminShopsPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  ...
  const status = filters.includes(selected as typeof filters[number]) ? selected : "all";
  ...
  return <div className="p-8"><div className="mb-8"><p className="text-[11px] font-black uppercase tracking-[.18em] text-orange-500">Marketplace control</p><h1 className="mt-2 text-3xl font-black">Shop management</h1><p className="mt-1 text-slate-500">Review vendor requests and control storefront availability.</p></div>
    <div className="mb-5 flex flex-wrap gap-2">{filters.map((filter) => <a key={filter} href={filter === "all" ? "/admin/shops" : `/admin/shops?status=${filter}`} className={`rounded-lg px-4 py-2 text-sm font-bold capitalize ${status === filter ? "bg-orange-500 text-white" : "bg-white text-slate-600 shadow-sm dark:bg-slate-900 dark:text-slate-300"}`}>{filter}</a>)}</div>
