...
export default async function ProductPage({ params }: { params: Params }) {
  ...
  const [{ data: reviewRows }, reviewEligibility] = await Promise.all([
    supabase.from("product_reviews").select("id, rating, review, created_at").eq("product_id", product.id).eq("status", "approved").order("created_at", { ascending: false }),
    user ? supabase.rpc("can_review_product", { target_user_id: user.id, target_product_id: product.id }) : Promise.resolve({ data: false }),
  ]);
