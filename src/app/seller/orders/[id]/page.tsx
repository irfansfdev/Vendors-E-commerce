...
export default async function SellerOrderDetailPage({ params }: { params: Params }) {
  ...
  const customerName = String(profileRow.full_name ?? parentRow.customer_name ?? parentRow.full_name ?? parentRow.customer_id ?? "Customer");
  const customerPhone = String(profileRow.phone ?? parentRow.phone ?? parentRow.customer_phone ?? "");
  const addressText = [
  ...
  const addressDisplay = addressText || "Shipping details were submitted with this order.";
  return <main className="mx-auto max-w-[1000px] px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div className="surface p-6"><div className="flex items-center gap-3"><UserRound className="size-5 text-orange-500" /><h2 className="font-black">Customer information</h2></div><dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2"><div><dt className="text-xs text-slate-400">Name</dt><dd className="mt-1 font-bold">{customerName}</dd></div><div><dt className="text-xs text-slate-400">Phone</dt><dd className="mt-1 font-bold">{customerPhone || "Provided at checkout"}</dd></div><div><dt className="text-xs text-slate-400">Customer ID</dt><dd className="mt-1 break-all font-bold">{String(parentRow.customer_id ?? "-")}</dd></div><div className="sm:col-span-2"><dt className="text-xs text-slate-400">Delivery address</dt><dd className="mt-1 font-bold leading-6">{addressDisplay}</dd></div></dl></div></section>
