...
import { assignRiderAction, updateSellerOrderStatus } from "@/app/seller/orders/actions";
...
export default async function SellerOrderDetailPage({ params }: { params: Params }) {
  ...
  const [{ data: riders }, { data: assignment }] = await Promise.all([
    supabase.from("delivery_profiles").select("id,full_name,vehicle_type,vehicle_number").eq("status", "approved").order("full_name"),
    supabase.from("delivery_assignments").select("rider_id,status").eq("shop_order_id", id).maybeSingle(),
  ]);
  ...
  return <main className="mx-auto max-w-[1000px] px-4 py-8 sm:px-6 lg:px-8">
    ...
    <aside className="surface h-fit p-6"><h2 className="font-black">Update status</h2><p className="mt-1 text-xs leading-5 text-slate-500">The customer will see this status in tracking.</p><form action={updateSellerOrderStatus} className="mt-5 space-y-4"><input type="hidden" name="orderId" value={id} /><select name="status" defaultValue={currentStatus} className="field">{statuses.map((status) => <option key={status} value={status}>{status[0].toUpperCase() + status.slice(1)}</option>)}</select><button className="button-primary w-full bg-orange-500 hover:bg-orange-600">Save status</button></form><div className="mt-7 border-t border-slate-100 pt-6 dark:border-white/10"><h2 className="font-black">Assign rider</h2><p className="mt-1 text-xs leading-5 text-slate-500">Only approved riders can receive this shop order.</p>{riders?.length ? <form action={assignRiderAction} className="mt-4 space-y-3"><input type="hidden" name="orderId" value={id} /><select name="riderId" defaultValue={String(assignment?.rider_id ?? "")} className="field" required><option value="">Choose rider</option>{riders.map((rider) => <option key={String(rider.id)} value={String(rider.id)}>{String(rider.full_name)}{rider.vehicle_type ? ` · ${String(rider.vehicle_type)}` : ""}</option>)}</select><button className="button-secondary w-full">{assignment ? "Reassign rider" : "Assign rider"}</button></form> : <p className="mt-4 text-xs text-slate-500">No approved riders are available yet.</p>}</div></aside></div>
  </main>;
}
