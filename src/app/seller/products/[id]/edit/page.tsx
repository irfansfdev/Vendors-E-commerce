...
export default async function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="mx-auto max-w-[1220px] px-4 py-8 sm:px-6 lg:px-8"><Link href="/seller/products" className="inline-flex items-center gap-1 text-xs font-bold text-slate-500 hover:text-orange-500"><ChevronLeft className="size-4" /> Products</Link><div className="mb-8 mt-5"><p className="text-[11px] font-black uppercase tracking-[.18em] text-orange-500">Catalog management</p><h1 className="mt-2 text-4xl font-black tracking-[-.055em]">Edit product</h1><p className="mt-2 text-sm text-slate-500">Update every detail, category, image, option, and inventory value.</p></div><ProductForm productId={id} product={product as Record<string, unknown>} categories={(categories ?? []).map((category) => ({ id: String(category.id), name: String(category.name) }))} variants={normalizedVariants.map((variant) => ({ ...variant, attributes: variant.attributes }))} images={(images ?? []).map((image) => String(image.image_url))} /></main>;
}
