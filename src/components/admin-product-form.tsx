...
export function ProductForm(props: ProductFormProps) {
  ...
  const [variants, setVariants] = useState<Variant[]>(props.variants?.map((item) => ({ id: item.id, name: item.name ?? "", sku: item.sku, price: String(item.price ?? ""), stock: String(item.stock ?? ""), attributes: attributesToArray(item.attributes) })) ?? []);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setVariants(variantRows.map((item) => ({ id: String(item.id), name: String(item.name ?? ""), sku: String(item.sku ?? ""), price: String(item.price ?? ""), stock: String(item.stock_quantity ?? ""), attributes: attributesToArray(item.attributes) })));
  ...
  async function save(event: React.FormEvent) {
    ...
    const skuValues = variants.map((item) => item.sku.trim().toLowerCase()).filter(Boolean);
    ...
    try {
      ...
      if (variants.length) { const variantResult = await supabase.from("product_variants").upsert(variants.map((item) => ({ id: item.id ?? crypto.randomUUID(), product_id: id, name: variantName(item), sku: item.sku.trim(), price: item.price.trim() ? Number(item.price) : Number(price), stock_quantity: item.stock.trim() ? Math.max(0, Number(item.stock)) : 0, attributes: attributesToObject(item.attributes) })), { onConflict: "id" }); if (variantResult.error) throw variantResult.error; }
