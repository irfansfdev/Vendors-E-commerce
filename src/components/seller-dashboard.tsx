...
export function SellerDashboard({ shop, products, orders, payouts, earnings, orderItems, role }: Props) {
  ...
  return <main className="mx-auto max-w-[1100px] px-4 py-2 sm:px-6 lg:px-8">
    ...
    <section className="surface mt-6 overflow-hidden"><div className="flex items-center justify-between border-b border-slate-100 p-6 dark:border-white/10"><div><p className="text-[11px] font-black uppercase tracking-[.16em] text-orange-500">Latest activity</p><h2 className="mt-1 text-xl font-black">Recent orders</h2></div><Link href="/seller/orders" className="text-xs font-bold text-orange-500">See all</Link></div>{ordersWithTotals.length === 0 ? <p className="p-10 text-center text-sm text-slate-500">No orders yet. New orders will appear here.</p> : <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="border-b border-slate-100 bg-slate-50/70 text-[10px] font-black uppercase tracking-[.12em] text-slate-400 dark:border-white/10 dark:bg-white/5"><tr><th className="px-5 py-4">Order</th><th className="px-5 py-4">Customer</th><th className="px-5 py-4">Items</th><th className="px-5 py-4">Amount</th><th className="px-5 py-4">Payment</th><th className="px-5 py-4">Status</th><th className="px-5 py-4">Date</th><th className="px-5 py-4">Action</th></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-white/10">{ordersWithTotals.slice(0, 2).map((order) => <tr key={String(order.id)} className="transition hover:bg-orange-50/40 dark:hover:bg-white/5"><td className="px-5 py-4 font-semibold">#{String(order.id).slice(0, 8)}</td><td className="px-5 py-4 font-medium">{String(order.customer_name ?? order.customer_email ?? "Customer")}</td><td className="px-5 py-4">{orderItems.filter((item) => String(item.shop_order_id) === String(order.id)).reduce((total, item) => total + Number(item.quantity ?? 0), 0)}</td><td className="px-5 py-4 font-semibold">{formatCurrency(orderAmount(order))}</td><td className="px-5 py-4 text-slate-500">{String(order.payment_method ?? order.payment_status ?? "-").replaceAll("_", " ")}</td><td className="px-5 py-4"><span className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-black uppercase ${["delivered", "completed"].includes(orderStatus(order)) ? "bg-emerald-50 text-emerald-700" : "bg-orange-50 text-orange-700"}`}>{orderStatus(order).replaceAll("_", " ")}</span></td><td className="whitespace-nowrap px-5 py-4 text-slate-500">{order.created_at ? new Date(String(order.created_at)).toLocaleDateString() : "-"}</td><td className="px-5 py-4"><Link href={`/seller/orders/${String(order.id)}`} className="font-bold text-orange-600 hover:text-orange-700">View</Link></td></tr>)}</tbody></table></div>}</section>
  </main>;
}
