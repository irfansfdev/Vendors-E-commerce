...
function isPaid(order: Row) {
  return ["paid", "partially_refunded"].includes(String(order.payment_status ?? "").toLowerCase());
}
...
function getDailySales(orders: Row[]) {
  ...
  return Array.from({ length: 7 }, (_, index) => {
    ...
    return {
      ...
      amount: orders.filter((order) => isPaid(order) && String(order.paid_at ?? order.created_at ?? "").slice(0, 10) === key).reduce((sum, order) => sum + orderAmount(order), 0),
    };
  });
}
...
export function SellerDashboard({ shop, products, orders, payouts, role }: Props) {
  ...
  const metrics: Array<{ icon: LucideIcon; value: string | number; label: string; detail: string }> = [
    { icon: CircleDollarSign, value: formatCurrency(sales), label: "Gross sales", detail: `${paidOrders.length} paid orders` },
  ...
  return <main className="mx-auto max-w-[1100px] px-4 py-2 sm:px-6 lg:px-8">
    ...
    <section className="surface mt-6 overflow-hidden"><div className="flex items-center justify-between border-b border-slate-100 p-6 dark:border-white/10"><div><p className="text-[11px] font-black uppercase tracking-[.16em] text-orange-500">Latest activity</p><h2 className="mt-1 text-xl font-black">Recent orders</h2></div><Link href="/seller/orders" className="text-xs font-bold text-orange-500">See all</Link></div>{orders.length === 0 ? <p className="p-10 text-center text-sm text-slate-500">No orders yet. New orders will appear here.</p> : <div className="divide-y divide-slate-100 dark:divide-white/10">{orders.slice(0, 2).map((order) => <Link href={`/seller/orders/${String(order.id)}`} key={String(order.id)} className="flex items-center justify-between gap-4 p-5 transition hover:bg-orange-50/50 dark:hover:bg-white/5"><div><p className="font-extrabold">Order #{String(order.id).slice(0, 8)}</p><p className="mt-1 text-xs capitalize text-slate-500">{orderStatus(order)}</p></div><p className="font-black">{formatCurrency(orderAmount(order))}</p></Link>)}</div>}</section>
  </main>;
}
